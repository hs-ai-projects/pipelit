...
**版本号建议**：按 `rules/version-bump.md` 决策树执行，**按优先级自上而下匹配，命中即返回**。

核心流程：
1. 运行静态扫描（公共导出删除 / API 路径删除）
2. 检测 commit 类型（BREAKING CHANGE / feat / fix）
3. 输出一行 log：`[1.3-version] 版本号建议：<major/minor/patch>，命中规则：<规则名>，警告=<有/无>，建议版本=<vX.Y.Z>`
4. 写入 audit JSON（通过 `decision_log.py phase <version> 1.3`）
5. 若有警告，Phase 2 预览必须显示 ⚠️ + 提供升级选项
